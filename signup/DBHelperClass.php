...
class DBHelperClass {
	...
	function addParticipant($inputValues, $key) {
		...
		$prepStmt = "INSERT INTO `t9hacks_participants` 
				(`key`, `name`, `email`, `linkedin`, `resume`, `website`, `github`, `company`, `title`, `facebook`, `twitter`, `shirt`) 
				VALUES (:key, :name, :email, :linkedin, :resume, :website, :github, :company, :title, :facebook, :twitter, :shirt)";
		...
		$stmt->bindParam(':title', 		$inputValues['title']);
	...
	function addMentor($inputValues, $key) {
		// prepare statement
		$prepStmt = "INSERT INTO `t9hacks_register_mentors` 
				(`key`, `name`, `email`, `company`, `position`, `breakfast`, `lunch`, `dinner`, `area_web_design`, `area_web_dev`, `area_android`, `area_ios`, `area_uiux`, `area_gaming`, `area_print`, `area_arduino`)  
				VALUES (:key, :name, :email, :company, :title, :breakfast, :lunch, :dinner, :area_web_design, :area_web_dev, :area_android, :area_ios, :area_uiux, :area_gaming, :area_print, :area_arduino)";
		$stmt = $this->conn->prepare($prepStmt);
		$stmt->bindParam(':key', 			$key);
		$stmt->bindParam(':name', 			$inputValues['name']);
		$stmt->bindParam(':email', 			$inputValues['email']);
	...
	function getParticipants() {
		...
		$stmt = $this->conn->prepare("SELECT * FROM `t9hacks_register_participants`"); 
	...
	function getMentors() {
		// get all mentors
		$stmt = $this->conn->prepare("SELECT * FROM `t9hacks_register_mentors`"); 
